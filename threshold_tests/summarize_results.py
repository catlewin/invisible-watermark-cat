...
def summarize_upscale_results(
    ...
    for method in methods:
        ...
        for file in os.listdir(method_dir):
            if file.endswith("_upscale_results.csv"):
                file_path = os.path.join(method_dir, file)
                try:
                    df = pd.read_csv(file_path)
                    if df.empty:
...
def summarize_denoise_results(
    results_root: str,
    methods: List[str],
    colors: Dict[str, str],
    attack_name: str = "denoise"
):
    os.makedirs(results_root, exist_ok=True)
    summary_data = []
    ...
    for method in methods:
        ...
        for file in os.listdir(method_dir):
            if file.endswith("_denoising_results.csv"):
                file_path = os.path.join(method_dir, file)
                try:
                    df = pd.read_csv(file_path)
                    if df.empty:
                    ...
                    # Find denoise row
                    denoise_row = df[df["attack_type"] == "upscale"] # currently accidentally listed as upscale instead of denoise in the csv files
                    if denoise_row.empty:
                        continue
                    success = denoise_row["success"].iloc[0]
                    if success:
                        decode_success_count += 1
                    else:
                        decode_fail_count += 1
    ...
    for _, row in df.iterrows():
        ...
        plt.figure(figsize=(5, 4))
        bars = plt.bar(["Success", "Failure"], [successes, failures], color=[colors.get(method, "gray"), "lightcoral"])
        plt.title(f"{method} Denoise Decode Success")
        plt.ylabel("Number of Images")
        plt.ylim(0, max(successes + failures, 1) + 1)
        ...
        plt.tight_layout()
        out_path = os.path.join(results_root, f"{method}_denoise_decode_bar.png")
        plt.savefig(out_path)
        plt.close()
        print(f"Saved individual plot for {method} to {out_path}")
    ...
    md_path = os.path.join(results_root, f"{attack_name}_summary.md")
    with open(md_path, "w") as f:
        f.write(f"# ✨ Denoise Decode Summary\n\n")
        f.write("This summary reports how many images each method successfully decoded after image denoising.\n\n")
        f.write("| Method | Total Images | Clean Failures | Decode Success | Decode Failure |\n")
        f.write("|--------|---------------|----------------|----------------|----------------|\n")
        for _, row in df.iterrows():
            f.write(
        ...
        for method in df["method"]:
            f.write(f"### {method} Decode Results\n")
            f.write(f"![{method} Bar Graph]({method}_denoise_decode_bar.png)\n\n")
