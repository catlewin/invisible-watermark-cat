...
def test_brightness_increase_threshold(
    image_path: str,
    method: str,
    output_dir: str = "threshold_tests/increase_brightness_test_results",
    watermark: str = "qingquan",
    brightness_range: list[float] = None,
):
    if brightness_range is None:
        brightness_range = [1.0, 1.2, 1.4, 1.6, 1.8, 2.0]  # up to and including 2.0
    ...
    # CSV output
    csv_path = os.path.join(method_dir, f"{base_name}_increase_brightness_results.csv")
    with open(csv_path, mode="w", newline="") as f:
        ...
        print(f"\nTesting Brightness Increase on {base_name} using {method}...")
...
def batch_test_brightness_increase(
        ...
        for file in files:
            if file.lower().endswith((".jpg", ".jpeg", ".png")):
                ...
                for method in methods:
                    ...
                    test_brightness_increase_threshold(
...
if __name__ == "__main__":
    batch_test_brightness_increase(
        image_root="unsplash_test_set",
        methods=["dwtDct", "dwtDctSvd"],
        output_dir="threshold_tests/original_img_dwt_methods_results/increase_brightness_test_results"
    )
