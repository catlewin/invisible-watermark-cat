...
def embed_images(
    ...
    csv_path = os.path.join(method_dir, f"{base_name}_denoising_results.csv")
    with open(csv_path, mode="w", newline="") as f:
        writer = csv.writer(f, escapechar='\\', quoting=csv.QUOTE_NONNUMERIC)
        writer.writerow([
            "denoise", "decoded", "success", "attack_type", "can_decode_clean",
...
def decode_denoising_images(
    ...
    for method in methods:
        ...
        for image_name in os.listdir(method_dir):
            ...
            attacked_path = os.path.join(image_dir, f"{image_name}_denoising.jpg")
            ...
            csv_path = os.path.join(method_dir, f"{image_name}_denoising_results.csv")
            with open(csv_path, mode="a", newline="") as f:
                writer = csv.writer(f, escapechar='\\', quoting=csv.QUOTE_NONNUMERIC)
                writer.writerow([
                    "denoise",          # upscale ratio
